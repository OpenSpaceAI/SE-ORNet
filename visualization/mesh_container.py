...
class MeshContainer(object):
    ...
    def __init__(self, vert=None, face=None):
        if vert is None and face is None:
            return
        ...
        if face is not None and face.shape[0] == 3:
            face = face.transpose()
    ...
    def _load_from_file_mat(self, file_path, dataset="faust_reg"):
        ...
        if dataset == "tosca":
        ...
        elif "null" in file_path or (
            "partial" in file_path and "rescaled" not in file_path
        ):
            ...
            face = np.array(mesh_mat["model_remesh"]["triv"][0, 0].astype(np.int32))
        else:
    ...
    def _load_from_file_obj(self, file_path):
        ...
        return self._load_from_file_ply(os.path.basename(file_path)[:-4] + ".ply")
    ...
    def save_to_ply_and_obj(self):
        vertex = np.array(
            [tuple(i) for i in self.vert], dtype=[("x", "f4"), ("y", "f4"), ("z", "f4")]
        )
        face = np.array(
    ...
    def save_as_mat(self, file_path=""):
        face_to_save = self.face if (np.min(self.face) > 0) else (self.face + 1)
    ...
    def compute_LBO(self, num_evecs=15):
        spectrum_results = fem_laplacian(
            self.vert,
            self.face,
            num_evecs,
...
if __name__ == "__main__":
    a = MeshContainer().load_from_file(
        "/home/dvir/repos/DG2N/data/datasets/smal/dataset_cats/cats0.ply"
    )
