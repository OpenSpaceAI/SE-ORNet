...
from visualization.mesh_visualization_utils import create_colormap
...
class MeshVisualizer(object):
        ...
        with fig.batch_update():
            for idx in range(len(pcs)):
                ...
                self.set_fig_settings(
                    scene=eval(f"fig.layout.scene{idx+1 if idx>0 else ''}"),
                    fig=fig,
                    camera=camera,
                    scale=self.scale,
                )
    ...
    def visualize_pcs_same_fig(
        ...
        for idx, pc in enumerate(pcs):
            fig.add_trace(
                self.plotly_mesh(
                    MeshContainer(vert=pc),
                    color_map_vis=np.stack(
                        [pcs_color[idx] for i in range(pc.shape[0])]
                    ),
                    mesh_or_pc="pc",
                    use_text=use_text,
                ),
    ...
    def visualize_mesh_pair(
        ...
        if fwd_or_bac == "fwd":  # forward mapping
            color_map_vis = (
                ...
                else create_colormap(target_mesh.vert)
            )
        else:  # backward mapping
            color_map_vis = (
                color_map
                if color_map is not None
                else create_colormap(source_mesh.vert)
            )
        ...
        if (
            fwd_or_bac == "fwd"
        ):  # in forward mapping, the target is colored by the ground truth colors, and the source to target mapping is colored accordingly
        ...
        with fig.batch_update():
            ...
            # scale = np.max((np.max(np.abs(target_mesh.vert),0),np.max(np.abs(source_mesh.vert),0)),0)
            self.set_fig_settings(
                scene=fig.layout.scene, fig=fig, camera=camera, scale=self.scale
            )
            self.set_fig_settings(
                scene=fig.layout.scene2, fig=fig, camera=camera, scale=self.scale
            )
    ...
    def set_fig_settings(scene, fig, camera, width=600, height=600, scale=False):
        fig.layout.update(
            autosize=False, width=width, height=height, margin=dict(l=0, r=0, t=0, b=0)
        )
    ...
    @staticmethod
    def visualize_scalar_vector_on_shape_static(
        verts,
        face,
        scalar_map,
        display_up=False,
        ...
        return MeshVisualizer(
            dataset="faust", display_up=display_up
        ).visualize_scalar_vector_on_shape(
    ...
    def visualize_scalar_vector_on_shape(
        ...
        if normals is None:
            ...
            fig = go.Figure(
                data=[self.plotly_normals(points=source_mesh.vert, normals=normals)]
            )
        camera = dict(up=dict(x=0, y=1.0, z=0), eye=dict(x=-0.0, y=-0.0, z=5))
